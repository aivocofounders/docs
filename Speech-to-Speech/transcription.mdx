...
- **Rate:** 0.1 units per minute of audio
- Deducted from admin's **Vispark Cloud** account

### User Credits

- **Rate:** `cost_transcription` from user profile per minute
- Default: 0 (configurable per user)

**Example:**

- 2 min audio → 0.2 admin units + 1.0 user credits (if cost_transcription = 0.5)
